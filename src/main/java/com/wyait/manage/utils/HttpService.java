...
//		requestConfig.
	...
	public HttpResult doPostHeader(String url, Map<String, Object> param,String token)
			throws Exception {
...
//		requestConfig.
		...
		if (param != null) {
			...
			postData.put("btype", param.get("btype"));
			postData.put("mobileContentList", param.get("mobileContentList"));
			// 构造一个form表单式的实体,并且指定参数的编码为UTF-8
			UrlEncodedFormEntity formEntity = new UrlEncodedFormEntity(
					parameters, "UTF-8");
			// 将请求实体设置到httpPost对象中
			formEntity.setContentType((new BasicHeader(HTTP.CONTENT_TYPE, "application/json")));
			httpPost.setEntity((new StringEntity(postData.toString(),"application/json", HTTP.UTF_8)));
		}
